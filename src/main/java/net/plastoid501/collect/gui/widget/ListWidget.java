...
public class ListWidget extends ElementListWidget<ListWidget.Entry> {
    ...
    private void initEntries(MinecraftClient client, String listName) {
        ...
        if (config != null) {
            ...
            for (ItemStack stack : Configs.collectItems.getStacks().get(listName)) {
    ...
    public class CategoryEntry extends Entry {
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            this.categoryText.setPos(x + 170, y + 5);
            this.categoryText.render(matrices, mouseX, mouseY, tickDelta);
        }
    ...
    public class ItemEntry extends Entry {
        ...
        ItemEntry(Text itemId, ItemStack stack, TextRenderer textRenderer, ModConfig config, String listName) {
            ...
            this.addButton = ButtonWidget.builder(Text.literal("Add"), button -> {
                if (!ItemUtil.contains(Configs.collectItems.getStacks().get(this.listName), stack)) {
            ...
            this.removeButton = ButtonWidget.builder(Text.literal("Remove"), button -> {
                int index = ItemUtil.indexOf(Configs.collectItems.getStacks().get(this.listName), stack);
                if (index != -1) {
            ...
            this.addButton.active = !ItemUtil.contains(Configs.collectItems.getStacks().get(this.listName), stack);
            this.removeButton.active = ItemUtil.contains(Configs.collectItems.getStacks().get(this.listName), stack);
        ...
        @Override
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ListWidget.this.itemRenderer.renderInGui(this.stack, x + 18, y + 2);
            if (x + 18 <= mouseX && mouseX <= x + 18 + 16 && y + 2 <= mouseY && mouseY <= y + 2 + 16) {
                ListWidget.this.parent.renderTooltip(matrices, ListWidget.this.parent.getTooltipFromItem(this.stack), mouseX, mouseY);
        ...
        @Override
        void update() {
            if (ItemUtil.contains(Configs.collectItems.getStacks().get(this.listName), this.stack)) {
