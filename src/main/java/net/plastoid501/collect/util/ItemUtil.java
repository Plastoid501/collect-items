...
public class ItemUtil {
    public static void moveItems(Screen screen, int fromSlot, int toSlot){
        ...
        ((IHandledScreenMixin) handler).collect_items$onMouseClick(handler.getScreenHandler().getSlot(fromSlot), fromSlot, 0, SlotActionType.PICKUP);
        ((IHandledScreenMixin) handler).collect_items$onMouseClick(handler.getScreenHandler().getSlot(toSlot), toSlot, 0, SlotActionType.PICKUP);
    }

    public static void quickMoveItems(Screen screen, int fromSlot) {
        HandledScreen<? extends ScreenHandler> handler = (HandledScreen<? extends ScreenHandler>) screen;
        ((IHandledScreenMixin) handler).collect_items$onMouseClick(handler.getScreenHandler().getSlot(fromSlot), fromSlot, 0, SlotActionType.QUICK_MOVE);
    }

    public static void throwItems(Screen screen, int fromSlot) {
        HandledScreen<? extends ScreenHandler> handler = (HandledScreen<? extends ScreenHandler>) screen;
        ((IHandledScreenMixin) handler).collect_items$onMouseClick(handler.getScreenHandler().getSlot(fromSlot), fromSlot, 1, SlotActionType.THROW);
    }
    ...
    public static void rightPickupItems(Screen screen, int fromSlot) {
        HandledScreen<? extends ScreenHandler> handler = (HandledScreen<? extends ScreenHandler>) screen;
        ((IHandledScreenMixin) handler).collect_items$onMouseClick(handler.getScreenHandler().getSlot(fromSlot), fromSlot, 1, SlotActionType.PICKUP);
    }
